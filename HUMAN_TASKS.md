...
- Paste your real Web3Forms access key into `siteConfig.contact.web3formsAccessKey` (currently `"YOUR_WEB3FORMS_ACCESS_KEY"`).
- File: `src/config/site.config.ts` → `contact.web3formsAccessKey`
- Send it to me in chat (or paste it directly into the file yourself) and I'll wire it in — Web3Forms access keys are meant to be public-facing in HTML/JS, so this is safe to share.
...
## 4. About / bio
- Real bio copy (currently two TODO placeholder paragraphs).
- File: `src/config/site.config.ts` → `about.bio`

## 5. Social links / profile URLs
